...
function getSelectedRedFlags(answers) {
...
function scorePatterns(answers) {
...
function answeredCount(answers) {
    return Object.values(answers).filter((value) => {
        if (value === null || value === undefined || value === '' || value === 'not_sure')
            return false;
        if (Array.isArray(value))
            return value.length > 0 && !value.includes('not_sure');
        return true;
    }).length;
}
function confidenceFor(score, totalAnswered) {
    if (score >= 7 && totalAnswered >= 12)
        return 'fair';
    if (score >= 4 && totalAnswered >= 8)
        return 'medium';
    return 'low';
}
function detectMissingQuestions(answers, topScores) {
...
function safePatternWording(pattern, score, confidence) {
...
export function interpretYhocAnswers(answers) {
...
export function getYhocEngineDebug(answers) {
